...
namespace Ogre
{
	...
	MovieLogic::~MovieLogic()
	{
		...
		if( mSoundSystem )
		{
			...
			SoundManager::stopSoundManager( mSoundSystem );
		}

		//Unloaded any loaded sound modules
		SoundManager::unloadSoundModules();
	}
	...
	void MovieLogic::initialise()
	{
		...
		std::vector<std::string> list = SoundManager::loadSoundModules();
		if( list.size() != 0 )
		{
			...
			mSoundSystem = SoundManager::startUpSoundManager(list[0]);
	...
	{
		SoundManager::stopSoundManager( mSoundSystem );
		mSoundSystem = SoundManager::startUpSoundManager(sndMgr);
